/**
 * Sidebar picker for importing a Muse CLI session into the current project.
 *
 * Run: npm run test:renderer -- --test-name-pattern "Muse session import"
 */
...
describe("Muse session import picker", () => {
  ...
  it("shows an empty state when no Muse sessions exist", async () => {
    const m = await mount(
      <Sidebar
        {...sidebarProps({
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    await m.click(m.query("[data-import-muse-session]"));
    await m.flush();
    const empty = m.query("[data-cli-session-empty]");
    assert.ok(empty, "empty copy renders");
    assert.match(empty!.textContent || "", /No Muse CLI sessions/i);
    m.unmount();
  });
  ...
  it("shows a list error with Retry", async () => {
    ...
    const m = await mount(
      <Sidebar
        {...sidebarProps({
          listCliSessions: async () => {
            calls += 1;
            if (calls === 1) throw new Error("XDG_DATA_HOME missing");
            return sessions;
          },
          importCliSession: async () => thread,
        })}
    ...
    assert.ok(error);
    assert.match(error!.textContent || "", /XDG_DATA_HOME missing/);
  ...
  it("keeps the picker open when import fails", async () => {
      ...
      <Sidebar
        {...sidebarProps({
          ...
          importCliSession: async () => {
            throw new Error("Muse session not found");
          },
        })}
      />,
    );
    ...
    const error = m.query("[data-cli-session-error]");
    assert.ok(error);
    assert.match(error!.textContent || "", /Muse session not found/);
    m.unmount();
  });
  ...
  it("does not render the menu item when the optional props are omitted", async () => {
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    assert.equal(m.query("[data-import-muse-session]"), null);
