/**
 * Sidebar picker for importing a Codex CLI session into the current project.
 *
 * Run: npm run test:renderer -- --test-name-pattern "CLI session import"
 */
...
describe("CLI session import picker", () => {
  it("lists sessions and imports the chosen one into the current project", async () => {
    ...
    const openBtn = m.query("[data-import-cli-session]");
  ...
  it("shows an empty state when no Codex sessions exist", async () => {
    const m = await mount(
      <Sidebar
        {...sidebarProps({
          listCliSessions: async () => [],
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    await m.click(m.query("[data-import-cli-session]"));
    await m.flush();
    const empty = m.query("[data-cli-session-empty]");
    assert.ok(empty, "empty copy renders");
    assert.match(empty!.textContent || "", /No Codex CLI sessions/i);
    m.unmount();
  });
  ...
  it("shows a list error with Retry", async () => {
    ...
    const m = await mount(
      <Sidebar
        {...sidebarProps({
          listCliSessions: async () => {
            calls += 1;
            if (calls === 1) throw new Error("CODEX_HOME missing");
            return sessions;
          },
          importCliSession: async () => thread,
        })}
    ...
    assert.ok(error);
    assert.match(error!.textContent || "", /CODEX_HOME missing/);
  ...
  it("keeps the picker open when import fails", async () => {
    ...
    const error = m.query("[data-cli-session-error]");
    assert.ok(error);
    assert.match(error!.textContent || "", /Codex session not found/);
    m.unmount();
  });
  ...
  it("does not render the menu item when the optional props are omitted", async () => {
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    assert.equal(m.query("[data-import-cli-session]"), null);
