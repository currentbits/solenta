/**
 * Sidebar picker for importing a Cursor CLI session into the current project.
 *
 * Run: npm run test:renderer -- --test-name-pattern "Cursor session import"
 */
...
describe("Cursor session import picker", () => {
  ...
  it("shows an empty state when no Cursor sessions exist", async () => {
    const m = await mount(
      <Sidebar
        {...sidebarProps({
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    await m.click(m.query("[data-import-cursor-session]"));
    await m.flush();
    const empty = m.query("[data-cli-session-empty]");
    assert.ok(empty, "empty copy renders");
    assert.match(empty!.textContent || "", /No Cursor CLI sessions/i);
    m.unmount();
  });
  ...
  it("shows a list error with Retry", async () => {
    ...
    const m = await mount(
      <Sidebar
        {...sidebarProps({
          listCliSessions: async () => {
            calls += 1;
            if (calls === 1) throw new Error("CURSOR_HOME missing");
            return sessions;
          },
          importCliSession: async () => thread,
        })}
    ...
    assert.ok(error);
    assert.match(error!.textContent || "", /CURSOR_HOME missing/);
  ...
  it("keeps the picker open when import fails", async () => {
      ...
      <Sidebar
        {...sidebarProps({
          ...
          importCliSession: async () => {
            throw new Error("Cursor session not found");
          },
        })}
      />,
    );
    ...
    const error = m.query("[data-cli-session-error]");
    assert.ok(error);
    assert.match(error!.textContent || "", /Cursor session not found/);
    m.unmount();
  });
  ...
  it("does not render the menu item when the optional props are omitted", async () => {
    ...
    await m.click(m.query("[data-new-thread-caret]"));
    assert.equal(m.query("[data-import-cursor-session]"), null);
