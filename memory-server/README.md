...
REST (same bearer auth): `GET /api/recent`, `GET /api/search`, `GET /api/entry/:id`, `POST /api/store`, `POST /api/session`, `GET /api/session-search`, `GET /api/bootstrap`, `GET /api/maintenance`, `POST /api/review/:id/resolve`.
