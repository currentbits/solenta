# Solenta - Concept Brainstorm

Solenta is a local-first desktop app for **running multi-agent software work with a visible pipeline**, not another chat box that happens to edit files. Electron + React. Dark three-pane shell: projects and threads on the left, the working thread in the center, and a live agent workflow graph on the right.
...
**One sentence:** Solenta turns "kick off an agent and hope" into a **directed multi-phase workflow** you can watch, stop, restage, and land as a PR.
...
Solenta sits in the gap: **you own the workflow graph**. A thread is not "a conversation with one model." It is a job with named phases (Seed → Analyze → Verify → Judge → Synthesize), fan-out inside phases, status per worker, token burn in the open, and a human still at the Build button.
...
## 2. How Solenta differs

| Axis | Claude Code / terminal agents | t3code | Synara | **Solenta** |
|------|-------------------------------|--------|--------|-----------|
...
### Project
A local git repository registered with Solenta. Owns path, default branch, preferred models, and a list of Threads. Opening a project does not mean "open a folder in an editor"; it means "this is a workspace agents may mutate under policy."
...
### PR
The preferred land path for a Thread: open or update a pull request from the thread branch. PR number and CI state surface as thread badges. Solenta is not a full GitHub client; it is good at "this work → reviewable PR."
...
Does Solenta own the phase graph and call models as leaf tools, or does it shell out to Claude Code / Codex as the real agent and only visualize? Opinion for v1: **own the graph, adapt the leaves.** Owning only visualization of foreign harnesses will always feel second-class.

### State and durability
Electron main process will spawn PTYs and manage worktrees. If the app crashes mid-run, what resumes? Threads and AgentRuns need durable state on disk (SQLite or JSON under `~/Library/Application Support/Solenta`). Open: resume policy for half-finished phases.

### Competitive gravity
t3code and synara move fast and already have multi-agent + worktree stories. Solenta wins only if the **pipeline metaphor** is clearer and more trustworthy, not if we match feature checklists. Risk: building a clone with different branding.
...
A developer opens Solenta, picks a project, starts a thread: "Add rate limiting to the public API and open a PR." Solenta seeds context, fans out analyze workers on the relevant packages, verifies with tests in isolated worktrees, judges which approach wins, synthesizes a single branch, and surfaces **PR #128** on the left with a Done badge. The right pane shows every phase settled. The token meter is non-zero and unsurprising. The human never had five terminal tabs and a spreadsheet of PIDs.
