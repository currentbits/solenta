...
/**
 * #834: classifyTool on ssh/WSL kimi turns.
 *
 * #671 isolates KIMI_CODE_HOME locally. runner.js skips that overlay when
 * crossesBoundary() and wrapCommand does not forward KIMI_CODE_HOME, so a
 * deny-tier tool can run on the remote before any Guardrail notice.
 * Official kimi docs: [[hooks]] event = "PreToolUse" in config.toml,
 * relocated by KIMI_CODE_HOME. These tests fail if that hole remains.
...
describe("resolveSpawn kimi KIMI_CODE_HOME across a boundary", () => {
...
emit({
  ...
  session_id: "kimi-gr-ssh",
  command: "kimi -r kimi-gr-ssh",
  content: "To resume this session: kimi -r kimi-gr-ssh",
});
process.exit(0);
`,
  );
...
describe("kimi runner: deny-tier tool on a crossesBoundary turn", () => {
  it("blocks curl|sh over ssh: no execute, Guardrail event, no control_request", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "coder-kimi-ssh-gr-"));
    ...
    try {
      ...
      assert.equal(
        seen.executed,
        false,
        `deny-tier curl|sh executed on the ssh kimi turn with no hook block: ${JSON.stringify(seen)}`,
      );
