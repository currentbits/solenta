...
describe("spendByDay and settings", () => {
  ...
  it("settings default null; set validates and persists without touching threads", () => {
    ...
    assert.deepEqual(store.getSettings(), { dailyBudgetUsd: null, orchestrationBudgetUsd: null, autoSettleAfterDays: 3, mcpServers: [], defaultWorktree: false, defaultOrchestrate: false, updateChannel: null, notifications: true, agentProfiles: [] });
    assert.deepEqual(services.getSettings(store), { dailyBudgetUsd: null, orchestrationBudgetUsd: null, autoSettleAfterDays: 3, mcpServers: [], defaultWorktree: false, defaultOrchestrate: false, updateChannel: null, notifications: true, agentProfiles: [] });

    const set = services.setSettings(store, { dailyBudgetUsd: 12.5 });
    assert.deepEqual(set, { dailyBudgetUsd: 12.5, orchestrationBudgetUsd: null, autoSettleAfterDays: 3, mcpServers: [], defaultWorktree: false, defaultOrchestrate: false, updateChannel: null, notifications: true, agentProfiles: [] });
    assert.equal(store.getThreads()[0].updatedAt, 2);

    store.saveNow();
    const reloaded = new Store(filePath);
    assert.deepEqual(reloaded.getSettings(), { dailyBudgetUsd: 12.5, orchestrationBudgetUsd: null, autoSettleAfterDays: 3, mcpServers: [], defaultWorktree: false, defaultOrchestrate: false, updateChannel: null, notifications: true, agentProfiles: [] });

    const cleared = services.setSettings(store, { dailyBudgetUsd: null });
    assert.deepEqual(cleared, { dailyBudgetUsd: null, orchestrationBudgetUsd: null, autoSettleAfterDays: 3, mcpServers: [], defaultWorktree: false, defaultOrchestrate: false, updateChannel: null, notifications: true, agentProfiles: [] });
